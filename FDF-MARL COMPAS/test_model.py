...
# ------------------------
# File paths
fairness_file = "BASE - fairness_metrics.csv"
performance_file = "BASE - performance_metrics.csv"
...
with open(fairness_file, mode='w', newline='') as file1, open(performance_file, mode='w', newline='') as file2:
    ...
    for ep in range (50):
        # -------- CONFIG --------
        MODEL_PATH = f"BASE FL Models/ep{ep}/Server.keras"
        # Load dataset
        df = pd.read_csv(DATA_PATH)
        X, y = df.iloc[:,:-2].astype('float32').values, df.iloc[:,-2:].astype('float32').values

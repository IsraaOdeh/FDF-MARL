...
agentsFile = f"AR-adults-keras_testing_DI.csv"
episodesFile = f"TR-adults-keras_testing_DI.csv"
...
with open(agentsFile, mode='w', newline='') as file1, open(episodesFile, mode='w', newline='') as file2:
    ...
    for ep in range(num_episodes):
        ...
        Path(f"FL Models/ep{ep}/").mkdir(parents=True, exist_ok=True)

        for i  in env.agents:
           env.models[i].save(f"FL Models/ep{ep}/{i}.keras")

...
agentsFile = f"AR-adults-keras_agg{CP}.csv"
episodesFile = f"TR-cadults-keras_agg{CP}.csv"

path = f'saved_models_adults_agg_{CP-200}/'
...
def average_models(model_paths):
    """
    Loads multiple models from model_paths and returns a new model with averaged weights.
    """
    models = [load_model(path) for path in model_paths]
    
    # Clone the first model to keep the architecture
    avg_model = clone_model(models[0])
    avg_model.build(models[0].input_shape)  # Ensure it's built
    avg_model.compile(optimizer='adam', loss='mse')  # or your actual config

    # Get all weights and average them
    ...
    for ep in range(num_episodes):
      ...
      # Saving models after every 10 episodes
        if ep%CHECKPOINTS == 0 and ep!=0:
            ...
            avg_ac = average_models([save_path + 'actor' + str(a) + '.h5' for a in range(num_agents)])
            avg_cr = average_models([save_path + 'critic' + str(a) + '.h5' for a in range(num_agents)])
            avg_target_ac = average_models([save_path + 'target_actor' + str(a) + '.h5' for a in range(num_agents)])
            avg_target_cr = average_models([save_path + 'target_critic' + str(a) + '.h5' for a in range(num_agents)])

            for k in range(num_agents):
                ac_models[k] = clone_model(avg_ac)
                cr_models[k] = clone_model(avg_cr)
                target_ac[k] = clone_model(avg_target_ac)
                target_cr[k] = clone_model(avg_target_cr)
                
                ac_models[k].set_weights(avg_ac.get_weights())
                cr_models[k].set_weights(avg_cr.get_weights())
                target_ac[k].set_weights(avg_target_ac.get_weights())
                target_cr[k].set_weights(avg_target_cr.get_weights())
